...
alter table Emp add index idx_Emp_email (email);

alter table Emp modify column mobile varchar(13) not null default '' after salary;
update Emp set mobile=concat('010-', lpad(id, 4, '0'), '-', lpad(id, 4, '0'));
alter table Emp add index idx_Emp_mobile (mobile);

select mobile from Emp where mobile like '010%';
select mobile from Emp where substring_index(mobile, '-', -1);
select * from emp where substring(mobile, 10, 4) = '0252';
explain select * from emp where substring(mobile, 10, 4) = '0252';

alter table Emp add index(
	(substring(mobile, 10, 4))
);

select * from Emp limit 100, 10;
select * from Emp limit 10 offset 100;

explain select * from Emp where email like 'mail1%';
select * from Emp where email like 'mail1%';
...
show index from Emp;
alter table Emp drop index idx_Emp_email;
...
alter table Notice add constraint fk_Notice_wirter_Emp foreign key (writer)
  references Emp(id) on delete set null;
...
select * from Notice where match(title, contents) against('조선* -국왕*' IN BOOLEAN MODE);
...
select * from TestEmp;
explain select * from TestEmp;

ALTER TABLE TestEmp MODIFY COLUMN id INT NOT NULL AUTO_INCREMENT,
	ADD PRIMARY KEY (id)

Partition by RANGE(TestEmp) (
    partition pa1 values less than(100),
    partition pa2 values less than(200),
